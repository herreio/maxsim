...
cat("fetch topic model of max planck research publications..\n")
model <- readRDS(gzcon(url(MODEL)))
model <- model$btm_all
usethis::use_data(model, overwrite=TRUE)
rm(model)

cat("\nfetch max planck research publications and its topics..\n")
items <- readRDS(gzcon(url(ITEMS)))
items <- items$btm_t500
usethis::use_data(items, overwrite=TRUE)
rm(items)

cat("\nfetch topic distribution of max planck research publications..\n")
